...
const createAjaxErrorHandler = (config: AjaxErrorHandlerConfig): DefaultErrorHandler => {
    return (
        status: number,
        error: Error,
        ...
        if (status > 0 && (error as AxiosError).response) {
            const response: AxiosResponse | undefined = (error as AxiosError)
                .response;
            if (response) {
                const responseMsg = config.responseMessageExtractor?.(response) ?? '';
                const fullResponseMsg = responseMsg ? `Response Message: ${responseMsg}` : '';
                const errorMsg = getFullErrorMessage(requestMessage || '', error);
                const fullMsg = `${errorMsg} Status: ${status}`;
